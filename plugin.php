	...
	class HTTP_Digest_Auth_plugin
	{
		...
		function plugin_activate()
		{
			...
			foreach( $users as $user )
			{
				...
				if( empty( $username ) && empty( $password ) && empty( $anyone ) ) :
					...
					add_user_meta( $user->ID, $this->slug.'_password', $user->user_pass );
					//add_user_meta( $user->ID, $this->slug.'_password', $this->encrypt( $user->user_login, $user->user_pass ) );
		...
		function user_credentials()
		{
			...
			foreach( $users as $user )
			{
				...
				$check = hash_equals( $user->user_pass, $password);
				//$check = $users->user_pass == $password;
				$credentials[$username] = array();
				$credentials[$username]['password'] = $check ? $user->user_pass : $this->decrypt( $user->user_login, $password );
				$credentials[$username]['type']		= $check ? "wp-password" : "modified-password";
		...
		function HTTP_Digest_Authenticate()
		{
			...
			$this->users = $this->user_credentials();
			
			//self::PrintR($this->digestParts);
			self::PrintR($this->users);
			//self::PrintR($this->slug);
			
			if ( ! isset( $this->users[ $this->digestParts['username'] ] ) )
				$this->requireLogin( $realm, $nonce );

			// Based on all the info we gathered we can figure out what the response should be
			$password = $this->get_password($this->digestParts['username']);
			$A1 = md5( $this->digestParts['username'] . ':' . $realm . ':' . $password );
		...
		public function HTTP_Basic_Auth(){
			...
			$realm = $_SESSION['unique_realm'];
			
			$auth = $this->basicAuth();
			
			// If there was no basicAuth, show login
			if ( is_null( $auth ) ){
				$this->requireBasicLogin( $realm, $auth );
			}
		}
		
		//Get password to match for http-digest-autentication
		function get_password($user_login){
			$password = null;
			if(is_array($this->users[$user_login]) && isset($this->users[$user_login]['password'])){
				$user = $this->users[$user_login];
				$password = $user['type'] == "wp-password" ? $user['password'] : $this->encrypt($user_login, $$user['password'] );
			}
			return $password;
		}
		...
		public function getBasicAuth(){
			...
			if (isset($_SERVER['PHP_AUTH_USER'])) {
				$basicAuth['username'] = $_SERVER['PHP_AUTH_USER'];
				$basicAuth['password'] = $_SERVER['PHP_AUTH_PW'];

			// most other servers
			} elseif (isset($_SERVER['HTTP_AUTHORIZATION'])) {

					if (strpos(strtolower($_SERVER['HTTP_AUTHORIZATION']),'basic')===0)
					  list($basicAuth['username'],$basicAuth['password']) = explode(':',base64_decode(substr($_SERVER['HTTP_AUTHORIZATION'], 6)));
		...
		public function requireBasicLogin($realm, $auth)
		{
		...
		function add_user_credentials( $user_ID )
		{
			...
			if( empty( $username ) && empty( $password ) && empty( $anyone ) ) :
				...
				add_user_meta( $user->ID, $this->slug.'_password', wp_hash_password("password") );
				//add_user_meta( $user->ID, $this->slug.'_password', $this->encrypt( $user->user_login, 'password' ) );
				add_user_meta( $user->ID, $this->slug.'_anyone', '0' );
			endif;
		...
		public static function PrintR($vars)
		{
			echo "<pre>";
			print_r($vars);
			echo "</pre>";
